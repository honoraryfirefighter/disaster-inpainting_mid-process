...
# 배경 인페인팅 함수를 정의합니다.
def background_inpainting(image_path, disaster_type):
    # 모델 설정
    model_path = 'C:/Users/admin/Desktop/seg inpainting/clipseg_weights/clipseg_weights/rd64-uni.pth'  # 모델 가중치 경로를 적절히 설정하세요.
    model = CLIPDensePredT(version='ViT-B/16', reduce_dim=64)
    model.eval()
    model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')), strict=False)
    
    # Stable Diffusion 인페인팅 파이프라인 설정
    model_dir = "stabilityai/stable-diffusion-2-inpainting"  # 적절한 모델 디렉토리 설정이 필요합니다.
    ...
    clipseg_prompt = 'sky' if disaster_type in ['태풍', '호우'] else 'ground'
    with torch.no_grad():
    ...
    # 인페인팅 수행
    inpainting_prompt = "dark and cloudy sky" if disaster_type in ['태풍', '호우'] else "flooded ground"
...
# 이 함수를 main.py에서 다음과 같이 호출할 수 있습니다.
# final_image = background_inpainting('path/to/image.jpg', '태풍')
